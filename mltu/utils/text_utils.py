...
def beam_decoder(predictions, lexicon, tokens, lm=None, nbest=10, beam_size=1500, lm_weight=3.23, word_score=-0.26):
    beam_ctc_decoder = ctc_decoder(lexicon=lexicon, tokens=tokens, lm=lm,
        nbest=nbest, beam_size=beam_size, lm_weight=lm_weight, word_score=word_score,)

    preds = beam_ctc_decoder(torch.from_numpy(predictions))

    texts, scores = [], []
    for pred in preds[0]:
        text = ["".join([tokens[idx] for idx in pred.tokens])][0]
        texts.append(text[1:-1])
        scores.append(pred.score)
    # texts = ["".join([tokens[idx] for idx in preds[0][0].tokens])]

    return texts, scores

def my_ctc_decoder(predictions: np.ndarray, chars: typing.Union[str, list]) -> typing.List[str]:
